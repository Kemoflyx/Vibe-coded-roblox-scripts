...
local function startRLock(target) 
    ...
    rLockConn = trackActive(RunService.PreRender:Connect(function(dt)
        ...
        local predPos = rLockTarget.Position + (_rld and Vector3.new(_rld.velocity.X, 0, _rld.velocity.Z) * settings.Prediction or Vector3.zero)
        ...
        if rawDir.Magnitude > 0.01 then
            ...
            local lerpSpeed = math.clamp(settings.LerpMin + spd * settings.LerpVel, settings.LerpMin, settings.LerpMax)
